...
test.describe('測定ワークフロー', () => {
  ...
  test('完全な測定ワークフローが正常に動作する', async () => {
    ...
    await expect(page.locator('[data-testid="camera-preview"]')).toBeVisible({ timeout: 10000 });
    
    // カメラ初期化の完了を待つ
    await page.waitForSelector('[data-testid="camera-status"][data-status="ready"]', { timeout: 15000 });
    
    // 3. 手首測定開始ボタンをクリック
    const startButton = page.locator('[data-testid="start-measurement-button"]');
    await expect(startButton).toBeEnabled();
    await startButton.click();
    
    // 測定モードに入ったことを確認
    await expect(page.locator('[data-testid="measurement-status"]')).toHaveText('測定中...');
    
    // 4. 手の検出とランドマーク表示を確認
    await page.waitForSelector('[data-testid="hand-landmarks"]', { timeout: 10000 });
    ...
    await page.waitForFunction(() => {
      const angleValue = document.querySelector('[data-testid="current-angle-value"]');
      return angleValue && angleValue.textContent !== '---';
    }, { timeout: 5000 });
    
    const angleValue = await page.locator('[data-testid="current-angle-value"]').textContent();
    ...
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('測定データを保存しました');
    ...
    await expect(page.locator('[data-testid="saved-measurement-angle"]')).toContainText(angleValue || '');
    ...
    // 今日の測定が統計に反映されることを確認
    await expect(page.locator('[data-testid="total-measurements"]')).toContainText('1');
  });
  ...
  test('カメラアクセス拒否時のエラーハンドリング', async () => {
    ...
    // エラーメッセージが表示されることを確認
    await expect(page.locator('[data-testid="camera-error"]')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('[data-testid="camera-error"]')).toContainText('カメラへのアクセスが拒否されました');
    
    // 設定ページへのリンクが表示されることを確認
    await expect(page.locator('[data-testid="camera-settings-link"]')).toBeVisible();
  });

  test('オフライン状態での測定とデータ同期', async () => {
    // オンライン状態で測定開始
    await page.goto('/measurement');
    await page.waitForSelector('[data-testid="camera-status"][data-status="ready"]');
    ...
    await expect(page.locator('[data-testid="offline-notification"]')).toContainText('オフラインモード');
    
    // データがローカルに保存されることを確認
    await expect(page.locator('[data-testid="local-save-notification"]')).toBeVisible();
    
    // オンライン復帰
    await page.context().setOffline(false);
    
    // 同期通知が表示されることを確認
    await expect(page.locator('[data-testid="sync-notification"]')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-testid="sync-notification"]')).toContainText('データを同期しています');
  });
  ...
  test('複数回の測定セッション', async () => {
    ...
    await page.goto('/progress');
    await expect(page.locator('[data-testid="total-measurements"]')).toContainText('3');
    ...
    // 複数の測定記録が表示されることを確認
    await expect(page.locator('[data-testid="measurement-list"] [data-testid="measurement-item"]')).toHaveCount(3);
  });
  ...
  test('測定精度の一貫性チェック', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const angleText = await page.locator('[data-testid="current-angle-value"]').textContent();
