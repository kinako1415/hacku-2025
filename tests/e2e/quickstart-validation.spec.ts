...
describe('Quickstart検証シナリオ', () => {
  ...
  test('シナリオ1: 初回ユーザーの完全なフロー', async () => {
    ...
    await expect(page.locator('[data-testid="start-measurement-button"]')).toBeVisible();
    ...
    // 設定保存確認
    await expect(page.locator('[data-testid="setup-success-message"]')).toBeVisible();
    ...
    // 結果確認
    const measurementResult = page.locator('[data-testid="measurement-result"]');
    ...
    const savedData = await page.evaluate(async () => {
      ...
      return new Promise((resolve) => {
        db.onsuccess = () => {
          const transaction = db.result.transaction(['measurements'], 'readonly');
    ...
    // メモ追加
    await page.fill('[data-testid="notes-textarea"]', '初回測定完了。手順通りに実行できました。');
    
    // 記録保存
    await page.click('[data-testid="save-record"]');
    
    // 保存確認
    await expect(page.locator('[data-testid="record-save-success"]')).toBeVisible();
  ...
  test('シナリオ3: 測定精度検証フロー', async () => {
    ...
    const landmarkCount = await page.locator('[data-testid="landmark"]').count();
    expect(landmarkCount).toBe(21);
    
    // 検出信頼度確認
    const confidence = await page.textContent('[data-testid="detection-confidence"]');
    ...
    for (const expectedAngle of testAngles) {
      ...
      const calculatedAngle = await page.textContent('[data-testid="calculated-angle"]');
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const result = await page.textContent('[data-testid="measurement-result"]');
    ...
    // 異なる測定タイプでの確認
    const measurementTypes = ['wrist-flexion', 'wrist-extension', 'thumb-abduction'];
  ...
  test('エラー処理とリカバリー検証', async () => {
    ...
    // エラーメッセージ表示確認
    await expect(page.locator('[data-testid="camera-error-message"]')).toBeVisible();
    
    // リトライボタンの確認
    await expect(page.locator('[data-testid="retry-camera-button"]')).toBeVisible();
