...
AI 駆動手首・母指可動域リハビリテーションアプリ - MediaPipe Hands/Pose を使用したカメラベース可動域測定システム
...
- `interface`よりも`type`を優先（Union types 対応）
- 型アサーション(`as`)は最小限に
- Generics の適切な活用
...
- App Router 使用（`page.tsx`, `layout.tsx`）
...
// Derived atomの活用
export const isReadyToMeasureAtom = atom(
  (get) => get(cameraStateAtom).isReady && get(mediaPipeStateAtom).isLoaded
);
```
...
- MediaPipe 読み込み失敗時の再試行ロジック
- オフライン状態の検出と対応

### テスト戦略

- **Contract Tests**: API 仕様準拠確認
