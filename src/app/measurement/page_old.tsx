...
const AngleOverlay: React.FC<{
  ...
  const handleResults = useCallback((results: Results) => {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      ...
      // 手が検出エリア内にあるかチェック
      const inDetectionArea = isHandInDetectionArea(landmarks, canvas.width, canvas.height);
...
const MeasurementPage: React.FC = () => {
  ...
  const handleAnglesUpdate = useCallback((angles: AngleData | null) => {
    setMeasurementState(prev => ({
      ...
      accuracy: angles ? Math.random() * 30 + 70 : 0, // 仮の精度計算
      lastUpdateTime: Date.now(),
    }));
  }, []);
      ...
      <div className={styles.content}>
        ...
        <div className={styles.controlsSection}>
          <MeasurementControls
            selectedHand={selectedHand}
            isCapturing={measurementState.isCapturing}
            currentAngles={measurementState.currentAngles}
            accuracy={measurementState.accuracy}
            handDetected={measurementState.handDetected}
