...
export const formattedAnglesAtom = atom((get) => {
  ...
  return {
    wrist: {
      ...
      radialDeviation: Math.round(measurementState.currentAngles.wrist.radialDeviation),
      ulnarDeviation: Math.round(measurementState.currentAngles.wrist.ulnarDeviation),
    },
    thumb: {
 ...
 * 測定停止用の書き込み可能アトム
 */
export const stopMeasurementAtom = atom(
  null,
  (get, set) => {
    const currentState = get(measurementStateAtom);
 ...
 * 精度更新用の書き込み可能アトム
 */
export const updateAccuracyAtom = atom(
  null,
  (get, set, accuracy: number) => {
    const currentState = get(measurementStateAtom);
 ...
 * 測定状態リセット用の書き込み可能アトム
 */
export const resetMeasurementStateAtom = atom(
  null,
  (get, set) => {
    set(measurementStateAtom, {
      isCapturing: false,
