...
const PhaseDisplay: React.FC<PhaseDisplayProps> = ({
  ...
  // 角度の正常範囲内かチェック
  const isAngleValid =
    currentAngle >= currentPhase.normalRange.min &&
  ...
  const angleProgress = Math.min(
    ...
    Math.max(
      0,
      ((currentAngle - currentPhase.normalRange.min) /
        (currentPhase.normalRange.max - currentPhase.normalRange.min)) *
        100
    )
  );
    ...
    <div className={styles.phaseDisplay}>
      ...
      <div className={styles.angleInfo}>
        <div className={styles.angleRange}>
          正常範囲: {currentPhase.normalRange.min}° -{' '}
          {currentPhase.normalRange.max}°
        </div>
        ...
        <div className={styles.angleProgress}>
          ...
          <div className={styles.rangeMarkers}>
            <span className={styles.minMarker}>
              {currentPhase.normalRange.min}°
            </span>
            <span className={styles.maxMarker}>
              {currentPhase.normalRange.max}°
            </span>
          </div>
        </div>
